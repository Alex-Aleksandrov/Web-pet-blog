package com.webapp.webblog.controllers;

import com.webapp.webblog.models.Post;
import com.webapp.webblog.repo.PostRepository;
...
public class BlogController {
    ...
    public String blogPostUpdate(@PathVariable(value = "id") long id, @RequestParam String title, @RequestParam String anons,
                                 @RequestParam String full_text, Model model) throws Exception {
        Post post = postRepository.findById(id).orElseThrow(() -> new Exception("не найдена статья с id " + id));
        post.setTitle(title);
        post.setAnons(anons);
